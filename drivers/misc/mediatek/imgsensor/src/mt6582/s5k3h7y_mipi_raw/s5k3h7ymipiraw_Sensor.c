...
{
	kal_uint16 frame_length = 0, line_length = 0, framerate = 0 , pixelclock = 0, frame_length_min = 0;	
	unsigned long flags;
...
static void S5K3H7YInitSetting(void)
{
  ...
  //1600x1200	
	...
	Sleep(10);//; delay(10ms)
#ifdef S5K3H7Y_OTP
printk("[S5K3H7Y] enter the otp config\n");
...
S5K3H7Y_bytewrite_cmos_sensor(0x3160,0x06);	// ash_GrasCfg  06  // 36  // [5:5]	fegras_gain_clamp   0 _ clamp gain to 0..1023 // _V_// 1 _ clamp_gain to 256..1023// [4:4]	fegras_plus_zero   Adjust final gain by the one or the zero // 0 _ [Output = Input x Gain x Alfa]  // _V_// 1 _ [Output = Input x (1 + Gain x Alfa)]
    ...
    //S5K3H7Y_bytewrite_cmos_sensor(0x0B01, 0x4F); // smiaRegs_rw_isp_luminance_correction_level 4F :85%  32:70%
    S5K3H7Y_bytewrite_cmos_sensor(0x0B01, 0x37); // smiaRegs_rw_isp_luminance_correction_level 4F :85%  32:70%
...
//	S5K3H7Y_bytewrite_cmos_sensor(0x0B01,0x00);	  // #smiaRegs_rw_isp_shading_correction_enable
...
//1600x1200
	...
	S5K3H7Y_wordwrite_cmos_sensor(0x0200,0x0618);	  // smiaRegs_rw_integration_time_fine_integration_time
	S5K3H7Y_wordwrite_cmos_sensor(0x0202,0x0979);	  // smiaRegs_rw_integration_time_coarse_integration_time
...
//1600x1200
	...
	S5K3H7Y_wordwrite_cmos_sensor(0x0202,0x0979);	// smiaRegs_rw_integration_time_coarse_integration_time (40ms)
	S5K3H7Y_wordwrite_cmos_sensor(0x0204,0x0020);	// X1
	S5K3H7Y_bytewrite_cmos_sensor(0x0B05,0x01  ); // #smiaRegs_rw_isp_mapped_couplet_correct_enable
#ifdef S5K3H7Y_OTP	
	S5K3H7Y_bytewrite_cmos_sensor(0x0B00,0x01  ); // #smiaRegs_rw_isp_shading_correction_enable
...
UINT32 S5K3H7YGetSensorID(UINT32 *sensorID)
{
	...
	s_DEVINFO_ccm->device_module = "OLQ8A16";//can change if got module id
	s_DEVINFO_ccm->device_vendor = "O-Film";
	s_DEVINFO_ccm->device_ic = "S5K3H7Y_MIPI";
	s_DEVINFO_ccm->device_version = "R1.0";
	s_DEVINFO_ccm->device_info = "800W";
	s_DEVINFO_ccm->device_used = DEVINFO_USED;

	DEVINFO_CHECK_DECLARE(s_DEVINFO_ccm->device_type,s_DEVINFO_ccm->device_module,s_DEVINFO_ccm->device_vendor,
				s_DEVINFO_ccm->device_ic,s_DEVINFO_ccm->device_version,s_DEVINFO_ccm->device_info,s_DEVINFO_ccm->device_used);
