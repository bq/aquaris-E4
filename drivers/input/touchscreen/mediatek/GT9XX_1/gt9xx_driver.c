...
static void devinfo_ctp_regchar(char *module,char * vendor,char *version,char *used)
{
	...
	s_DEVINFO_ctp->device_ic="GT968F";
...
static void gtp_pen_init(void)
{
    ...
    //input_set_abs_params(tpd->dev, ABS_MT_TRACKING_ID, 0, 255, 0, 0);//yzy
...
#if defined(LCT_ADD_TP_VERSION)
static int ctp_proc_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
...
static int ctp_proc_read(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	...
	if (tpd_info.manu_id == 0x00)
	{
		manu_string = "yushun";
	}
	else if (tpd_info.manu_id == 0x01)
	{
		manu_string = "dijing";
	}
	else
	...
	cnt = sprintf(page, "Manufacturer:%s, IC:GT%xF, Firmware:0x%x\n", manu_string, tpd_info.pid, tpd_info.vid);
*/
	cnt = sprintf(page,"pid:0x%x\n",tpd_info.vid);
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   // msleep(50);
	...
	temp_manuid = tpd_info.manu_id;
	sprintf(temp_ver,"0x%x",tpd_info.vid);

#ifdef SLT_DEVINFO_CTP_DEBUG
	printk("[DEVINFO CTP] rgister version:%s !\n",temp_ver);
#endif		
	switch(temp_manuid)
	{
		case 0x00:
			//printk("[DEVINFO CTP] rgisteri 0!\n");
			devinfo_ctp_regchar("unknown","yushun",temp_ver,DEVINFO_USED);
			break;
		case 0x01:
			//printk("[DEVINFO CTP] rgisteri 0!\n");
			devinfo_ctp_regchar("unknown","dijing",temp_ver,DEVINFO_USED);
			break;
		default:
			//printk("[DEVINFO CTP] unknown!\n");
			devinfo_ctp_regchar("unknown","unknown",temp_ver,DEVINFO_USED);
			break;
	}
#endif
