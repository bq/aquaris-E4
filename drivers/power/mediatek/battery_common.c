...
static void mt_battery_notify_VBatTemp_check(void)
{
...
#if 1
		if((BMT_status.temperature < MAX_CHARGE_TEMPERATURE_MINUS_X_DEGREE)&&(g_batt_temp_status == TEMP_POS_HIGH))
		{
    	#if 0 //defined(DISABLE_TEMP_NOTIFY)
		...
		else
		{
			...
			g_BatteryNotifyCode |= 0x0040;
	#endif
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) in range(normal)\n", BMT_status.temperature); 	
		}
		if((BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)&&(BMT_status.charger_exist == true))
		{
    	#if 0 //defined(DISABLE_TEMP_NOTIFY)
		if (g_battery_test_status == KAL_FALSE)
		{
		...
		else
		{
			...
			g_BatteryNotifyCode |= 0x0002;
	#endif
			battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) out of charge range(too high)\n", BMT_status.temperature);		
		}
