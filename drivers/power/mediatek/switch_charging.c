...
void select_charging_curret(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			if((BMT_status.temperature>10) && (BMT_status.temperature<40))
		{
			g_temp_input_CC_value = 80000;
			g_temp_CC_value = 65000;
		}
		else
		{
		   	g_temp_input_CC_value = 80000;
			g_temp_CC_value = 55000;
		}
...
#if 1
		kal_uint32 charging_ce_enable = KAL_FALSE;
		battery_charging_control(CHARGING_CMD_ENABLE_CE,&charging_ce_enable);
		battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] disable charging\n\r");  
	
		if(BMT_status.bat_vol < 4300)
			{
			charging_ce_enable = KAL_TRUE;
				battery_charging_control(CHARGING_CMD_ENABLE_CE,&charging_ce_enable); 
